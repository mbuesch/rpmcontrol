...
pub fn setup(c: &InitCtx) {
    ...
    let tc1 = DP_TC1.as_ref_with_initctx(c);
    tc1.tc1h().write(|w| w.set(0));
    tc1.tcnt1().write(|w| w.set(0));
    tc1.tccr1a().write(|w| w);
    tc1.tccr1c().write(|w| w);
    tc1.tccr1d().write(|w| w);
    tc1.tccr1e().write(|w| w);
    tc1.ocr1a().write(|w| w.set(0xFF));
    tc1.ocr1b().write(|w| w.set(0xFF));
    tc1.ocr1c().write(|w| w.set(0xFF)); // TOP value
    tc1.ocr1d().write(|w| w.set(0xFF));
    tc1.dt1().write(|w| w);
    tc1.tccr1b().write(|w| w.cs1().prescale_256());
}

#[inline(always)]
pub fn timer_get_cs(cs: CriticalSection<'_>) -> Timestamp {
    let tc1 = DP_TC1.as_ref_with_cs(cs);
    tc1.tcnt1().read().bits().into()
}
...
macro_rules! define_timer_interrupt {
    ($arm_fn:ident, $cancel_fn:ident, $irq_fn:ident, $handler_fn:path, $ocr:ident, $ocie:ident, $ocf:ident) => {
        pub fn $arm_fn(cs: CriticalSection<'_>, trigger_time: Timestamp) {
            let tc1 = DP_TC1.as_ref_with_cs(cs);

            // Ensure it doesn't trigger right away by pushing OCR into the future.
            let now_ticks: u8 = timer_get_cs(cs).into();
            tc1.tc1h().write(|w| w.set(0));
            tc1.$ocr().write(|w| w.set(now_ticks.wrapping_add(0x7F)));
            timer_sync_wait();

            // Clear trigger flag and enable interrupt.
            tc1.tifr().write(|w| w.$ocf().set_bit());
            tc1.timsk().modify(|_, w| w.$ocie().set_bit());

            // Program the compare register.
            tc1.tc1h().write(|w| w.set(0));
            tc1.$ocr().write(|w| w.set(trigger_time.into()));
            timer_sync_wait();
            let now = timer_get_cs(cs);

            // Trigger is in the past and has not triggered, yet?
            if trigger_time <= now && !tc1.tifr().read().$ocf().bit() {
                loop {
                    // Enforce trigger now.
        ...
        pub fn $cancel_fn(cs: CriticalSection<'_>) {
            let tc1 = DP_TC1.as_ref_with_cs(cs);
            tc1.timsk().modify(|_, w| w.$ocie().clear_bit());
            tc1.tifr().write(|w| w.$ocf().set_bit());
        }

        pub fn $irq_fn(c: &IrqCtx<'_>) {
            let tc1 = DP_TC1.as_ref_with_irqctx(c);
            tc1.timsk().modify(|_, w| w.$ocie().clear_bit());
            tc1.tifr().write(|w| w.$ocf().set_bit());
            let trig_time: Timestamp = tc1.$ocr().read().bits().into();
...
macro_rules! impl_reltimestamp {
    ($rel:ident, $abs:ident, $reltype:ty, $abstype:ty) => {
        ...
        impl $rel {
            ...
            #[inline]
            pub const fn to_micros(self) -> i32 {
                (self.0 as i32) * (TIMER_TICK_US as i32)
            }
            ...
            #[inline]
            pub const fn mul(self, d: $reltype) -> $rel {
                $rel(self.0 * d)
            }
